...
IpSocketAddress getLoopback(uint16_t Port) {
  IpSocketAddress A;
...
protected:
  void SetUp() override {
    ...
    IoInsts = newIoInstances(*Io);
    Sockets = std::make_unique<SocketsHost>(*Io);
    Insts = newSocketsInstances(*Sockets);
    Poll = IoInsts[1].get();
  ...
  std::optional<IpSocketAddress> bindTcp(uint64_t Rep) {
    EXPECT_TRUE(status(call(
        *Tcp, "[method]tcp-socket.start-bind",
        {tcp(Rep), network(),
         Runtime::Component::Wit<IpSocketAddress>::into(getLoopback(0))})));
    EXPECT_TRUE(
  ...
  std::optional<IpSocketAddress> bindUdp(uint64_t Rep) {
    EXPECT_TRUE(status(call(
        *Udp, "[method]udp-socket.start-bind",
        {udp(Rep), network(),
         Runtime::Component::Wit<IpSocketAddress>::into(getLoopback(0))})));
    EXPECT_TRUE(
  ...
  void writeBlocking(uint64_t OutRep, std::vector<uint8_t> Data) {
    ...
    ASSERT_TRUE(Res);
    EXPECT_TRUE(
        Runtime::Component::Wit<IoResult<Runtime::Component::WitUnit>>::from(
            (*Res)[0]));
  }
...
TEST_F(WasiP2Sockets, ConnectRefusedIsReported) {
  ...
  ASSERT_TRUE(status(
      call(*Tcp, "[method]tcp-socket.start-connect",
           {tcp(Client), network(),
            Runtime::Component::Wit<IpSocketAddress>::into(getLoopback(1))})));
