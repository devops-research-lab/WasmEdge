...
private:
  ...
  Expect<uint32_t> storeEvent(const Component::CanonFunction &Canon,
                              ...
                              uint64_t Ptr);
  /// Trap when a synchronous task may not block.
  Expect<void> checkMayBlock(Runtime::Component::Task &T,
                             Runtime::Component::Thread &Cur);
  /// Start the copy loaded on the role Role of S.
  Expect<void>
  startStreamCopy(Runtime::Instance::Component::StreamInstance &S,
                  Runtime::Instance::Component::StreamInstance::Role Role);
  /// Move Count elements from the writer of S to the reader.
  Expect<void> moveElements(Runtime::Instance::Component::StreamInstance &S,
                            uint32_t Count);
  ...
  /// Wait on the waitable set at SetIdx of Inst, between two callbacks of a
  /// callback loop when BetweenCallbacks.
  Expect<Component::WaitableEvent>
  waitOnWaitableSet(Runtime::Component::Task &T,
                    Runtime::Component::Thread &Cur,
                    const Runtime::Instance::ComponentInstance &Inst,
                    uint32_t SetIdx, bool Cancellable, bool BetweenCallbacks);
  ...
  /// \name The scheduler.
  /// @{
  /// Hand the stack to Target until it parks or ends.
  ...
  /// imports from until Done(); traps on a deadlock.
