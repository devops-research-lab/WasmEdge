...
inline Thread::WakeReason Task::onEnter(Thread &Cur) noexcept {
  ...
  if (FuncTypeAsync && !isHost()) {
    ...
    if (HasBackpressure() || Inst->getNumWaitingToEnter() > 0) {
      ...
      const Thread::WakeReason Wake = wait(
      ...
      if (Wake == Thread::WakeReason::Cancelled) {
        return Wake;
      }
    }
    if (needsExclusive()) {
...
inline bool Task::mayBlock(const Thread &Cur,
                           Span<Thread *const> Waiting) const noexcept {
  if (mayBlockAlways()) {
    return true;
  }
