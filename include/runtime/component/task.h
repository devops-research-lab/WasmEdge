...
public:
  ...
  /// Request cancellation: the thread to wake as cancelled, or null when it
  /// queues.
  Thread *onCancelRequest() noexcept {
    ...
    // Not started yet: the gated thread wakes as cancelled.
    ...
    for (Thread *Entry : Threads) {
      ...
      // No cancellation while another thread holds the stack exclusively.
  ...
  bool onCancelDelivery(bool Cancellable) noexcept {
    if (Cancellable && Status == TaskState::PendingCancel) {
      Status = TaskState::CancelDelivered;
      return true;
    }
    return false;
  ...
  /// the exclusive slot back and forget the threads, the instance and the
  /// callbacks, which may name frames that unwound.
  void onAbort() noexcept;

  /// The caller was aborted or its instance goes: its callbacks name frames
  /// and memory that are gone.
  ...
  Thread::WakeReason wait(Thread &Cur, std::function<bool()> Ready,
                          bool Cancellable) noexcept {
    return park(Cur, std::move(Ready), std::nullopt, {}, 0, Cancellable,
                /*InPlace=*/false, /*BetweenCallbacks=*/false);
  }
  /// Park Cur until Ready() holds, Until passes or an entry of Entries is
  /// ready.
  Thread::WakeReason
  waitAny(Thread &Cur, std::function<bool()> Ready,
    ...
    return park(Cur, std::move(Ready), Until, std::move(Entries), 0,
                /*Cancellable=*/true, /*InPlace=*/false,
  ...
  Thread::WakeReason collect(Thread &Cur, std::function<bool()> Ready,
                             uint32_t WaitingSet, bool Cancellable) noexcept {
    return park(Cur, std::move(Ready), std::nullopt, {}, WaitingSet,
                Cancellable, /*InPlace=*/true, /*BetweenCallbacks=*/false);
  }
  /// Park Cur ready to run again; it always cedes the stack.
  Thread::WakeReason yield(Thread &Cur, bool Cancellable) noexcept {
    return park(Cur, nullptr, std::nullopt, {}, 0, Cancellable,
                /*InPlace=*/false, /*BetweenCallbacks=*/false);
  }
  /// `collect` and `yield` of a callback loop between two callbacks.
  Thread::WakeReason collectCallback(Thread &Cur, std::function<bool()> Ready,
                                     uint32_t WaitingSet) noexcept {
    return park(Cur, std::move(Ready), std::nullopt, {}, WaitingSet,
                /*Cancellable=*/true, /*InPlace=*/true,
                /*BetweenCallbacks=*/true);
  }
  Thread::WakeReason yieldCallback(Thread &Cur) noexcept {
    return park(Cur, nullptr, std::nullopt, {}, 0, /*Cancellable=*/true,
                /*InPlace=*/false, /*BetweenCallbacks=*/true);
  }
  /// Park Cur with no condition.
  Thread::WakeReason suspend(Thread &Cur, bool Cancellable) noexcept {
    if (onCancelDelivery(Cancellable)) {
      return Thread::WakeReason::Cancelled;
    }
    return Cur.onSuspend(Cancellable);
  }
  /// Park Cur and name Other as the thread to run next.
  Thread::WakeReason switchTo(Thread &Cur, Thread &Other, bool IsYield,
                              bool Cancellable) noexcept {
    if (onCancelDelivery(Cancellable)) {
      return Thread::WakeReason::Cancelled;
    }
    return Cur.onSwitch(Other, IsYield, Cancellable);
  }
  /// Hand over to Other only if it is ready, else park Cur, ready again when
  /// IsYield.
  Thread::WakeReason promote(Thread &Cur, Thread &Other, bool IsYield,
                             bool Cancellable) noexcept {
    if (onCancelDelivery(Cancellable)) {
      return Thread::WakeReason::Cancelled;
    }
    return Cur.onPromote(Other, IsYield, Cancellable);
  }
  /// @}
  ...
  /// Whether blocking is unconditionally allowed.
  bool mayBlockAlways() const noexcept {
    return FuncTypeAsync || Status == TaskState::Resolved;
  }
  /// Whether Cur may block: another waiting thread of the tree is ready.
...
private:
  ...
  /// The one suspension point of a wait or a yield: the implicit thread
  /// hands the exclusive slot back while no guest frame of it runs.
  Thread::WakeReason
  park(Thread &Cur, std::function<bool()> Ready,
       std::optional<std::chrono::steady_clock::time_point> Until,
       std::vector<PollEntry> Entries, uint32_t WaitingSet, bool Cancellable,
    ...
    const bool Release = needsExclusive() && &Cur == ImplicitThread &&
                         hasExclusive() &&
                         (BetweenCallbacks || Status == TaskState::Resolved);
    if (Release) {
      releaseExclusive();
      if (Ready) {
    ...
    if (!InPlace || !Ready()) {
      Wake = Cur.onWait(std::move(Ready), Until, std::move(Entries), WaitingSet,
                        Cancellable);
    }
