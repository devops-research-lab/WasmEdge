...
public:
  ...
  Thread *onResume(WakeReason NewWake) noexcept {
    ...
    clearWait();
    Status = ThreadState::Running;
    Wake = std::exchange(CancelPending, false) && NewWake == WakeReason::Normal
               ? WakeReason::Cancelled
               : NewWake;
    return Root.resumeStack();
  }
  /// A suspended thread becomes ready (`thread.resume-later`).
  void onReady() noexcept { Status = ThreadState::Waiting; }
  /// The OS handle at Idx of its interests turned ready.
  void onIoReady(size_t Idx) noexcept { Interests[Idx].Ready = true; }
  /// A parked thread becomes ready and wakes as cancelled at its next resume.
...
protected:
  ...
  WakeReason onSuspend(bool IsCancellable) noexcept {
    return park(ThreadState::Suspended, IsCancellable, nullptr);
  }
  ...
  WakeReason onWait(std::function<bool()> Ready,
                    ...
                    std::vector<PollEntry> Entries, uint32_t SetIdx,
                    bool IsCancellable) noexcept {
    ReadyFunc = std::move(Ready);
    Deadline = Until;
    Interests = std::move(Entries);
    WaitingSet = SetIdx;
    return park(ThreadState::Waiting, IsCancellable, nullptr);
  }
  /// Park and name Other as the thread to run next; Other must be suspended.
  WakeReason onSwitch(Thread &Other, bool IsYield,
                      bool IsCancellable) noexcept {
  ...
  /// IsYield.
  WakeReason onPromote(Thread &Other, bool IsYield,
                       bool IsCancellable) noexcept {
    if (Other.isReady()) {
      return onSwitch(Other, IsYield, IsCancellable);
    }
    if (IsYield) {
      return onWait(nullptr, std::nullopt, {}, 0, IsCancellable);
    }
...
private:
  ...
  WakeReason park(ThreadState To, bool IsCancellable,
                  Thread *SwitchTo) noexcept {
