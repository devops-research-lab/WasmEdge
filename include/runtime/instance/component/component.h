...
public:
  ...
  virtual ~ComponentInstance() noexcept {
    ...
    if (Root == this) {
      ...
      if (Root->Entry == EntryKind::Outside) {
  ...
  /// Whether Other is this instance or a lexical ancestor or descendant.
...
protected:
  ...
  /// @{
  void setEntered(bool Flag) noexcept { Entered = Flag; }
  ...
  /// scheduler.
  /// @{
  /// The kind of the embedder entry in progress; Outside for none.
  ...
  /// Abort the tasks of this tree, the tasks sharing a stack with them and
  /// their callers up the call chain; IsPoisoning poisons the guest trees that
  /// lose a task.
  void abortTasks(bool IsPoisoning) noexcept {
    ...
    // still waiting to start has no arguments left and wakes as cancelled.
    for (auto *Store : Stores) {
    ...
    for (auto *T : Aborted) {
      // The unwinding skipped the exit of the instance it had entered, and a
      // guest tree that lost a task in the middle of a call is unusable.
    ...
    if (Terminating) {
      for (auto *Store : Stores) {
        if (Store != this && Store->Entry == EntryKind::Outside) {
...
private:
  ...
  EntryKind Entry = EntryKind::Outside;
