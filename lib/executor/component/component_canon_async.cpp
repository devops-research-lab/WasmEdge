...
ComponentExecutor::runCanonYield(const Component::CanonFunction &Canon,
  ...
  // A task that may not block yields in place but still takes a cancellation.
  const bool Cancellable = Canon.getFlag();
  if (!T->mayBlock(*Cur, Canon.getInstance()->getRoot()->Waiting)) {
    Rets[0] = ValVariant(T->onCancelDelivery(Cancellable) ? UINT32_C(1)
                                                          : UINT32_C(0));
    return {};
  }
  const Runtime::Component::Thread::WakeReason R = T->yield(*Cur, Cancellable);
  if (R == Runtime::Component::Thread::WakeReason::Aborted) {
    return Unexpect(ErrCode::Value::ComponentAsyncAborted);
  }
  Rets[0] = ValVariant(R == Runtime::Component::Thread::WakeReason::Cancelled
                           ? UINT32_C(1)
                           : UINT32_C(0));
  return {};
}
...
ComponentExecutor::runCanonSubtaskCancel(const Component::CanonFunction &Canon,
  ...
  if (!Callee->isResolved()) {
    if (Runtime::Component::Thread *Wake = Callee->onCancelRequest();
        Wake != nullptr) {
      resumeThread(*Wake, Runtime::Component::Thread::WakeReason::Cancelled);
    }
    if (const auto Trap =
    ...
    if (!Callee->isResolved()) {
      ...
      const Runtime::Component::Thread::WakeReason R = T->wait(
          *Cur,
          [Callee]() { return Callee->isResolved() || Callee->isAborted(); },
          /*Cancellable=*/false);
      if (R == Runtime::Component::Thread::WakeReason::Aborted) {
        return Unexpect(ErrCode::Value::ComponentAsyncAborted);
      }
...
// The copy of one role. See "include/executor/component/executor.h".
Expect<void> ComponentExecutor::startStreamCopy(
  ...
  const uint32_t N = S.onCopy(Role);
  if (N > 0) {
    EXPECTED_TRY(moveElements(S, N));
    S.onMove(N);
  }
  return {};
}
...
// "include/executor/component/executor.h".
Expect<void>
ComponentExecutor::moveElements(Runtime::Instance::Component::StreamInstance &S,
                                uint32_t Count) {
  auto &SrcBuffer =
...
Expect<uint32_t> ComponentExecutor::waitCopyResult(
  ...
  const Runtime::Component::Thread::WakeReason R = T->collect(
      *Cur, [&S, Role]() { return S.isCompleted(Role); }, 0,
      /*Cancellable=*/false);
  if (R == Runtime::Component::Thread::WakeReason::Aborted) {
    return Unexpect(ErrCode::Value::ComponentAsyncAborted);
  }
...
Expect<void> ComponentExecutor::runCanonStreamCopy(
  ...
  if (S.isEnded(Role)) {
    ...
    } else if (S.isEndedByPeerDrop(Role)) {
  ...
  // Two roles in one instance meet only over raw-copyable elements.
  if (S.isPeerPending(Role) && !S.getPeerBuffer(Role).IsHost &&
      S.getPeerBuffer(Role).Opts.Inst == Inst && Elem.has_value()) {
...
ComponentExecutor::runCanonStreamDrop(const Component::CanonFunction &Canon,
  ...
  if (!IsStream && !IsRead && !S.isEnded(Role) &&
      !S.isPeerHandleDropped(Role)) {
...
Expect<void> ComponentExecutor::runCanonWaitableSetWait(
  ...
  if (!IsPoll) {
    EXPECTED_TRY(
        const Component::WaitableEvent Ev,
        waitOnWaitableSet(*T, *Cur, *Inst, SetIdx, Cancellable, false));
  ...
  if (!Ev.has_value()) {
    if (T->mayBlock(*Cur, Canon.getInstance()->getRoot()->Waiting)) {
      const Runtime::Component::Thread::WakeReason R =
          T->yield(*Cur, Cancellable);
      if (R == Runtime::Component::Thread::WakeReason::Aborted) {
        return Unexpect(ErrCode::Value::ComponentAsyncAborted);
      }
      if (R == Runtime::Component::Thread::WakeReason::Cancelled) {
        Ev =
            Component::WaitableEvent{Component::EventCode::TaskCancelled, 0, 0};
      } else {
        Ev = FirstPending();
      }
    } else if (T->onCancelDelivery(Cancellable)) {
      Ev = Component::WaitableEvent{Component::EventCode::TaskCancelled, 0, 0};
    }
  }
  if (!Ev.has_value()) {
...
Expect<void> ComponentExecutor::runCanonThreadSwitch(
  ...
  if (IsPromote) {
    ...
    R = T->promote(*Cur, *Other, IsYield, Canon.getFlag());
  } else {
    ...
    R = T->switchTo(*Cur, *Other, IsYield, Canon.getFlag());
  }
  if (R == Runtime::Component::Thread::WakeReason::Aborted) {
    return Unexpect(ErrCode::Value::ComponentAsyncAborted);
  }
